...
define('SITE_ID', 'INGATLANBAZAR');
define('SITE_PREFIX', 'ingatlanbazar');
...
//empty list
if(ereg('<li>Nincs találat.</li>', $html)){ 
...
$hirdetesSum = 0;
$hirdetesek = $dom->find('li[class=ad]');
foreach($hirdetesek as $hirdetes){
	
	$hirdetesSum++;
	
	$link = $hirdetes->find('h3', 0)->find('a', 0)->href;
	if(!ereg('ingatlanbazar.hu', $link)) $link = 'http://www.ingatlanbazar.hu'.$link;
		
	$res = mysql_query("SELECT * FROM apro_hirdetesek_ingatlan WHERE hirdetesKod = '".$link."'");
	$data = mysql_fetch_assoc($res);
	if(!$data){
		
		$cim = $hirdetes->find('h3', 0)->find('a', 0)->plaintext;
		
		if($rowp['varos'] != 'Budapest'){ 
			$varos = $hirdetes->find('li[class=location]', 0)->innertext;
		}
		else{ 
			$varos = $rowp['varos'];
		}
		...
		if($varos == 'Budapest' || mysql_num_rows($res)){
			...
			$ar = $hirdetes->find('div[class=info]', 0)->find('strong', 0)->innertext;
			$ar = explode('<br />', $ar);
			$ar = preg_replace('/[^0-9]/', '', $ar[0]);
			
			$kep = $hirdetes->find('div[class=photo]', 0)->find('img', 0)->src;
			if($kep  && !ereg('nophotouploaded.gif', $kep)){
