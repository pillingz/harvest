...
foreach($hirdetesek as $hirdetes){
	...
	$res = mysql_query("SELECT * FROM apro_hirdetesek_tech WHERE hirdetesKod = '".$link."'");
	...
	if(!$data){
		...
		if($varos == 'Budapest' || mysql_num_rows($res)){
			...
			if($kep){
				...
				$fname = "uploads/tech/".SITE_PREFIX."/".$filename;
				$filename = "uploads/tech/".SITE_PREFIX."/".$filename;
			...
			mysql_query("
				INSERT INTO apro_hirdetesek_tech SET 
					hirdetesKod	= '".$link."', 
