...
foreach($hirdetesek as $hirdetes){
	...
	$res = mysql_query("SELECT * FROM apro_hirdetesek_allat WHERE hirdetesKod = '".$link."'");
	...
	if(!$data){
		...
		if($varos == 'Budapest' || mysql_num_rows($res)){
			...
			if($kep){
				...
				$fname = "uploads/allat/".SITE_PREFIX."/".$filename;
				$filename = "uploads/allat/".SITE_PREFIX."/".$filename;
			...
			mysql_query("
				INSERT INTO apro_hirdetesek_allat SET 
					hirdetesKod	= '".$link."', 
