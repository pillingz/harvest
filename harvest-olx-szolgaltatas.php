...
foreach($hirdetesek as $hirdetes){
	...
	$res = mysql_query("SELECT * FROM apro_hirdetesek_szolgaltatas WHERE hirdetesKod = '".$link."'");
	...
	if(!$data){
		...
		if(ereg('Budapest', $varos)){
			...
			$kerulet = explode('.', trim($kerulet[1]));
		...
		if($varos == 'Budapest' || mysql_num_rows($res)){
			...
			if($kep){
				...
				$fname = "uploads/szolgaltatas/".SITE_PREFIX."/".$filename;
				$filename = "uploads/szolgaltatas/".SITE_PREFIX."/".$filename;
			...
			mysql_query("
				INSERT INTO apro_hirdetesek_szolgaltatas SET 
					hirdetesKod	= '".$link."', 
